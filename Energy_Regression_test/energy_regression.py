...
tmp = file_name.split("/")
out_dir = tmp[-2] + "_" + tmp[-1].replace("-training.root", "_test2019049")
...
xgb_default = {'min_child_weight': 1,
               ...
               'reg_alpha': 0,
               'reg_lambda': 1}
output = open(out_dir+'/param_range.pkl', 'wb')
pickle.dump(hyperparams_ranges, output)
output.close()
output = open(out_dir+'/param_default.pkl', 'wb')
pickle.dump(xgb_default, output)
output.close()

df_train = load_data(file_name, entrystop=1500000, isEE=isEE)#the files have ~5M events, stop at 1.5M
...
xgbo_reg = XgboRegressor(out_dir, early_stop_rounds=100, num_rounds_min=250)

xgbo_reg.optimize(xgtrain, init_points=4, nfold=5, n_iter=50, acq='ei')
...
xgbo_default = xgb.Booster({'nthread':6}) #init model
...
xgbo_bo = xgb.Booster({'nthread':6}) #init model
